...
public class CapClipManager {
	...
	private static void updateDatabase(DataQuery query, List<HashMap<String, String>> table) {
		...
		for (HashMap<String, String> row : table) {
			if (engagementExists(row.get(CapClipField.ENGAGEMENT_ID.csvName))) continue;
			recordsAdded = true;
			...
			sb.append(" INTO ").append(TABLE_NAME).append(" (").append(columns).append(") VALUES (").append(values).append(") ");
		}
	...
	private static boolean engagementExists(String value) {
		if (value == null) return false;
		String q = "SELECT * FROM " + TABLE_NAME + " WHERE " + CapClipField.ENGAGEMENT_ID.dbName + " = '" + value + "'";
