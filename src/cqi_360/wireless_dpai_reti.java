...
public class wireless_dpai_reti {
...
Thread.sleep(5000);
 		...
 		driver.findElement(By.xpath("//*[@class='reviewDirTabs']/div[2]/div/div[2]/div[2]/div/div[4]/div/div[11]/div/div/div/a")).click();
		 
 		WebElement e1 = driver.findElement(By.xpath("//html/body/ul[@id='hoverdiv'][49]/li/a[@title='Trend']"));
 		e1.click();
       Thread.sleep(5000);
 		 js = (JavascriptExecutor) driver;
 		 js.executeScript("window.scrollBy(0,500)", "");
 		...
 		//Open downloaded file
            ...
            if (Desktop.isDesktopSupported()) {
                   ...
                   try {
                          ...
                          File myFile1 = new File("C:/Users/" + System.getProperty("user.name") + "/Downloads/eng-wireless-all-_-view-chart-2.xls");
        ...
        Thread.sleep(6000);
         ...
         myFile = new File("C:\\Users\\akvangal\\AppData\\Roaming\\Microsoft\\AddIns\\test.xls");
    	   ...
    	   String deletequery = "DELETE FROM OPSBI_360_RETI_DPAI_WIRELESS";
    	   ...
    	   for(int i=0;i< categoryList.size()*2;i++){	
   			...
   			if (j >= categoryList.size()) {
     			...
     			String query = "INSERT INTO OPSBI_360_RETI_DPAI_WIRELESS (REC_ID,QUERY_CD,CATEGORY_DATE,CR,SA,UT,RETI_ADOPTION,TOTAL_DEV_ESCAPE,TOTAL_TEST_ESCAPE) VALUES ('"+(i+1)+"','"+qry_code+"','"+categoryList.get(j)+"','"+CR_List.get(j)+"','"+SA_List.get(j)+"','"+UT_List.get(j)+"','"+RETI_List.get(j)+"','"+DEV_List.get(j)+"','"+Test_List.get(j)+"')";
    			System.out.println("");
    			smt.executeQuery(query).close();
    		}
    		
    	
    		System.out.println("--- ORACLE Data Table  Inserted into Table OPSBI_360_RETI_DPAI_WIRELESS---");
	}
		...
		public static void main(String args[]) throws Exception, InterruptedException, SQLException, IOException {
			wireless_dpai_reti dna1= new wireless_dpai_reti(); 
		dna1.set_cqa_DNA();
