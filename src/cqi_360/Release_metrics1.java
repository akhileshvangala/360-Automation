...
public class Release_metrics1 {
	...
	//myFile = new File("C:\\Users\\akvangal\\Downloads\\test.xls");
		...
		for(int i=0; i<dropdown1.size(); i++) {
			...
			Thread.sleep(1000);
			if(dropdown1.get(i).getText().equalsIgnoreCase("16101 Release View")) {
 	  ...
 	  String deletequery = "DELETE FROM OPSBI_360_RELEASE_METRICS2";
 	  smt.executeQuery(deletequery).close();
		...
		String query = "INSERT INTO OPSBI_360_RELEASE_METRICS2 (REC_ID,QUERY_CD,COUNT_RG,COUNT_CFD,COUNT_URC,MTTR_RG,MTTR_CFG,MTTR_URC) VALUES ('"+1+"','E2','"+rg+"','"+cfd+"','"+urc+"','"+mttr_rg+"','"+mttr_cfd+"','"+mttr_urc+"')";
		...
		smt.executeQuery(query).close();
 		...
 		System.out.println("--- ORACLE Data Table  Inserted into Table OPSBI_360_RELEASE_METRICS2---");
	...
	public static void main(String args[]) throws ClassNotFoundException, InterruptedException, SQLException, IOException, AWTException {
		Release_metrics1 rm= new Release_metrics1();
		rm.set_cqa_DNA();
	}
