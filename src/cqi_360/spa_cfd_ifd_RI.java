...
public class spa_cfd_ifd_RI {
	...
	//myFile = new File("C:\\Users\\akvangal\\Downloads\\test.xls");
 		...
 		//public void openAndCloseSummaryReportExcel() throws AWTException {
            ...
            if (Desktop.isDesktopSupported()) {
                   ...
                   try {
                          ...
                          File myFile1 = new File("C:/Users/" + System.getProperty("user.name") + "/Downloads/enb-spa-all-_-view-1.xls");
 	  ...
 	  String deletequery = "DELETE FROM OPSBI_360_CFD_IFD_RI_SPA";
 	  ...
 	  {	
		  ...
		  if (j >= category_IFD_List.size()) {
 			...
 			String query = "INSERT INTO OPSBI_360_CFD_IFD_RI_SPA (REC_ID,QUERY_CD,CATEGORY_DATE,CFD_INCOMING,CFD_DISPOSED,CFD_BACKLOG,CFD_AVERAGE,CFD_MTTR,IFD_AVERAGE,IFD_MTTR,IFD_BACKLOG,CFR,IFR) VALUES ('"+(i+1)+"','"+ qry_code +"','"+category_IFD_List.get(j)+"','"+CFD_IncomingList.get(j)+"','"+CFD_DisposedList.get(j)+"','"+CFD_BacklogList.get(j)+"','"+CFD_AverageList.get(j)+"','"+CFD_MttrList.get(j)+"','"+IFD_AverageList.get(j)+"','"+IFD_MttrList.get(j)+"','"+IFD_BacklogList.get(j)+"','"+CFR_List.get(j)+"','"+IFR_List.get(j)+"')";
 			System.out.println("ABCD");
 			smt.executeQuery(query).close();
 	  }
		
 	
 		System.out.println("--- ORACLE Data Table  Inserted into Table OPSBI_360_CFD_IFD_RI_SPA---");
	...
	public static void main(String args[]) throws Exception, InterruptedException, SQLException, IOException {
		spa_cfd_ifd_RI dna1= new spa_cfd_ifd_RI(); 
		dna1.ifd();
